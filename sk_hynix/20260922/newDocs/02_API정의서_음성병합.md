...
| 버전 | 1.0.0 |
| 작성일 | 2026-09-19 |
| Base URL | `http://<병합서버>:9995` |
| 인증 | 없음 (도커 네트워크 내부 통신) |
| Content-Type | `application/json` |
| Swagger | `http://<병합서버>:9995/docs` — Basic 인증 (계정은 별도 전달) |
...
정렬은 **요청값 하나로 정해지지 않는다.** 서버가 아래 순서로 시도하고,
실제로 적용된 방식을 응답의 `alignment.mode`로 돌려준다.

```
① 동기화 비프음 탐색   ← options.align 과 무관하게 항상 먼저
     ├─ 찾음   → 비프 지점 기준 정렬            mode: "beep"
     └─ 못 찾음 ↓
② options.align == "timestamp" → startedAt 기준   mode: "timestamp"
③ options.align == "none"      → 전부 0초에서 겹침  mode: "none"
```

### 6.1 `options.align` — ①이 실패했을 때의 동작
...
`"beep"`은 **요청값으로 넣을 수 없다.** 비프 탐색은 항상 수행되며, 찾으면 무조건 그쪽이 이긴다.
오디오 안에 있는 기준점이 기기가 보고한 시각보다 언제나 나은 증거이기 때문이다.

### 6.2 비프음 정렬 (①)

방장 폰이 스피커로 울린 동기화 비프음을 각 트랙의 앞 10초 구간에서 찾아,
그 지점이 같아지도록 맞춘다. 기기 시계·녹음 기동 지연·마이크 버퍼 지연을 전부 건너뛴다.

- 규격과 음원: `newDocs/04_비프음동기화_설계.md`, `newDocs/assets/sync_beep.wav`
- 합성 검증 기준 **정렬 오차 2ms**, 트랙당 0.11초
- 비프를 못 찾은 트랙은 `startedAt`이 있으면 그걸로, 없으면 0으로 배치하고 `fallback`에 남긴다
- 최소 2개 트랙에서 잡혀야 비프 정렬을 쓴다 (하나뿐이면 맞출 대상이 없다)
- `BEEP_ENABLED=0`으로 끌 수 있다. 끄면 예전처럼 ②·③만 동작한다

### 6.3 ⚠️ 비프음 정렬이면 결과 파일 앞이 잘린다 — `trimmedMs`

비프음으로 정렬했을 때는 **마지막 비프가 끝나는 지점까지 잘라내고** 결과를 만든다.
비프 소리와 회의 시작 전 구간이 함께 사라져, 결과 파일은 **회의가 시작되는 지점부터** 시작한다.

```
자르기 전 (합친 직후)
0초                    5.016초                         끝
├──────────────────────┼──────┬──────┬──────┬──────────┤
│  회의 시작 전 구간     │ 비프 │ 비프 │ 비프 │  회의     │
└──────────────────────┴──────┴──────┴──────┴──────────┘
                                            ↑ 7.366초
자른 결과 (실제 업로드되는 파일)
0초                                                    끝
...
```json
"alignment": { "mode": "beep", "beepAtMs": 5016, "trimmedMs": 7366, ... }
```

STT 결과의 타임스탬프를 원래 타임라인으로 되돌리려면 이 값을 더하면 된다.

```js
const originalMs = sttResultMs + (data.alignment?.trimmedMs ?? 0);
```

| 상황 | `trimmedMs` |
|---|---|
| `mode: "beep"` + 잘라냄 | 잘라낸 밀리초 (예: `7366`) |
| `mode: "beep"` + 안 잘라냄 (녹음이 짧아 취소됨) | `0` — 대신 비프 구간이 무음 처리된다 |
| `mode: "timestamp"` / `"none"` | **필드 없음** — 자르지 않는다 |

> 잘라낸 뒤 남는 길이가 5초 미만이면 자르기를 **취소**하고 비프 구간만 무음 처리한다.
> 비프가 늦게 잡힌 짧은 녹음에서 회의 내용까지 날아가는 것을 막는다.
>
> 서버 설정 `BEEP_TRIM=0`으로 자르기를 끌 수 있다. **요청 본문으로는 바꿀 수 없다** —
> 호출마다 달라지면 타임스탬프 환산이 일관되지 않기 때문이다.

### 6.4 `startedAt`은 밀리초 단위로 — ②를 쓸 때
...
> `output.durationMs`는 **잘라낸 뒤의 길이**다. 원본 트랙보다 `trimmedMs`만큼 짧다.
> (6.3절 참조)

`alignment.mode`는 **실제로 적용된** 방식이다. 요청한 `options.align`과 다를 수 있다 —
비프음을 찾으면 `"beep"`이 되기 때문이다.

| `mode` | 함께 오는 필드 |
|---|---|
| `"beep"` | `reference`, `beepAtMs`, **`trimmedMs`**, `tracks[]` (`offsetMs`, `beepAtMs`, `confidence`, `fallback`) |
| `"timestamp"` | `reference`, `tracks[]` (`offsetMs`, `fallback`) |
| `"none"` | 없음 — `{ "mode": "none" }` 만 |

`tracks[].fallback`은 `mode`에 따라 뜻이 다르다.

- `mode: "timestamp"` — `true`면 이상값 가드에 걸려 0으로 되돌려진 트랙
- `mode: "beep"` — 비프를 못 찾아 내려간 단계명(`"timestamp"` / `"none"` / `"outlier"`).
  정상이면 `null`
...
```bash
curl -X POST http://<서버>:9995/v1/merge/upload \
  -F "files=@p1.flac" \
  -F "files=@p2.flac" \
  -F "files=@p3.flac" \
  -F "align=timestamp" \
...
```bash
curl -o merged.flac http://<서버>:9995/v1/merge/mrg_.../download
```
...
```
Method : POST
URL    : http://<서버>:9995/v1/merge/upload
Body   : form-data
...
| 비프 검출 | Goertzel 이중음 (3000Hz + 4200Hz), 앞 10초 |
| 앞부분 잘라내기 | ffmpeg `atrim` + `asetpts` — 비프음 정렬일 때만 |
| 결과 길이 | 가장 긴 트랙 기준. 비프음 정렬이면 `trimmedMs`만큼 짧다 |
...
양쪽 모두 `timblo-net` 도커 네트워크에 속해야 한다.

| 항목 | 값 |
|---|---|
| 네트워크 | `timblo-net` (external) |
| 컨테이너명 | `aimm-recog-backend` |
| 포트 | 9995 (호스트) → 8080 (컨테이너) |
...
- [ ] **`alignment.trimmedMs` 처리** — 비프음 정렬이면 결과 파일 앞이 잘린다 (6.3절)
...
| 파일 암호화 | 없음 | 저장 시점엔 평문 |
| 정렬 | 비프음 우선 / `timestamp` / `none` 3단계 | 오디오 안의 기준점이 기기가 보고한 시각보다 나은 증거 |
| 비프음 정렬 시 앞부분 | 마지막 비프까지 잘라냄 | 비프 소리와 회의 전 구간을 한 번에 제거. `trimmedMs`로 되돌릴 수 있음 |
| 자르기 여부 | 서버 설정(`BEEP_TRIM`) | 호출마다 달라지면 타임스탬프 환산이 일관되지 않음 |
