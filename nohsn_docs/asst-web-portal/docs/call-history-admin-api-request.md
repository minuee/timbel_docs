...
| `direction` | 없음 | **인입유형 필터 추가 요청**. `I`(인바운드) / `O`(아웃바운드). 미지정 = 전체 |
...
| 필드 | 화면 표기 | 현재 |
|---|---|---|
| 통화 결과(완료·이관·콜백) | 통화결과 | 응답 필드 자체 없음 — 셀렉트·카드 모두 미구현 표기 |
| `intent` | 주요 의도 폴백 | 항상 null (`customer_inquiry` 로 대체 표시 중) |

> `direction`(인입유형)은 값이 실리기로 확정돼 프론트 연동을 마쳤다(카드 I/B·O/B 배지 + 상단 필터).
> 값이 아직 안 실린 콜은 카드에 `-` 로 떨어진다.
...
- [ ] `GET /callstat/call-history` — `direction`(I/O) 필터 지원 ※ 프론트 연동 완료, 파라미터만 받으면 됨
