...
| 필드 | 필요 여부 | 없으면 |
|---|---|---|
| `company.id` | 필요 | LLM 기반 기능 불가 (미해결 항목) |

**DTO 추가 필요** — 현재 받지 않는다.

### 5. `POST /postcall/**` — 통화 후 처리
...
### 6. `GET /agents?favorite_only=true` — 즐겨찾기 상담사 조회

| 필드 | 필요 여부 | 없으면 |
|---|---|---|
| `agent_id` | 필요 | 즐겨찾기 필터가 동작하지 않음 |

**쿼리 파라미터 추가 필요** — 현재 토큰으로 본인을 식별하고 있다.

> 주의: 이 값은 "누구의 즐겨찾기인가"를 정하는 키다. 프론트가 보내는 값을 그대로 믿으면
> 다른 상담사의 즐겨찾기를 조회할 수 있다. 감수하고 갈지 판단이 필요하다.

### 7. `GET /agents?assignable_*` — 배정 가능 상담사 목록

백엔드가 `GET {USER_HOST}/api/user/assignable` 을 직접 호출한다
(`UserInfoService.getAssignableAgents()`). get_user 는 아니지만 user-service 직접 호출이다.

**결정 필요** — 프론트가 직접 호출할지, 백엔드가 계속 호출할지.
