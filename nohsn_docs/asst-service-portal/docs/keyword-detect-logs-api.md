...
**`callstats_id` 안내** — 요약(`advisor.summary`), 감정(`advisor.emotions`), 할일(`advisor.todos`)이
공통으로 쓰는 통화 키(`raw_call.callstats_call.id`)입니다. 이력에서 통화 상세로 넘어갈 때 이 값을
쓰면 됩니다. 이력 테이블에는 저장돼 있지 않고, `call_id` 로 조회 시점에 찾아 채웁니다.
따라서 **`callstats_call` 에 아직 적재되지 않은 통화(진행 중이거나 적재 전)는 `null`** 입니다 —
`null` 인 행은 통화 상세 링크를 걸지 말고, 기존대로 `call_id` 만 표시하면 됩니다.
...
### 드릴다운: 특정 감지어가 걸린 건만 보기

감지어 목록에서 행을 클릭했을 때, 그 키워드가 걸린 이력만 조회하는 용도입니다.

```
GET /aicc/asst-service/keyword-detect-logs?keyword={키워드}&type={분류}&page=1&limit=20
```

키워드에 한글·공백이 들어가므로 **반드시 URL 인코딩**해야 합니다.

```js
const url =
  `/aicc/asst-service/keyword-detect-logs` +
  `?keyword=${encodeURIComponent('원금 보장')}` +
  `&type=forbiddenWord&page=1&limit=20`;

// 실제 요청 URL
// /aicc/asst-service/keyword-detect-logs
//   ?keyword=%EC%9B%90%EA%B8%88%20%EB%B3%B4%EC%9E%A5&type=forbiddenWord&page=1&limit=20
```

응답의 각 행에서 `callstats_id` 로 통화 상세를 연결하고, `turn_idx` 로 몇 번째 발화인지 표시하면
됩니다. (`callstats_id` 가 `null` 인 행은 아직 통화 통계가 적재되지 않은 건입니다)
...
- `keyword` 는 **부분일치**입니다. 정확히 그 키워드만 보려면 `type` 을 같이 넘기고,
  응답에서 `keyword` 값을 한 번 더 확인하는 편이 안전합니다.
- 통화 상세에서 **발화 원문**을 보려면 `raw_call.callstats_turn` 적재가 필요합니다.
  현재는 비어 있어 `call_id` + `turn_idx` 연결까지만 됩니다.
...
- **발화 원문은 내려가지 않습니다.** `callstats_id`(또는 `call_id`) + `turn_idx` 로 통화 상세와
  연결하는 구조입니다.
- **`callstats_id`** 도 `agent_id` 처럼 조회 시점에 붙입니다(`raw_call.callstats_call` 조인).
  통화 통계가 아직 적재되지 않았으면 `null` 입니다. 필터로는 받지 않습니다.
