...
#### 3.9. 알림 (`notification:alert`)

특정 외부 API 노드가 실행되면 서버에서 알림 이벤트를 발송합니다.
...
```typescript
{
  ...
  details: {
    nodeId: string;           // 실행된 노드 ID
    nodeName?: string;        // 노드 이름
    graphId: string;          // 그래프 ID
...
socket.on("notification:alert", (notification) => {
  ...
  showNotificationPopup({
    title: notification.title,
    message: notification.message,
    apiId: notification.details.apiId,
    statusCode: notification.details.statusCode,
  });
});
