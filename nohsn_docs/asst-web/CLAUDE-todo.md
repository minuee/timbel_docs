...
### 10. 리뉴얼 폰트 시스템 — 포털 dynamic 연동 완료, 다른 앱과 "완전 통일"은 선택 [적용·검증 완료 / 통일은 요청 시]
> ✅ **폰트 조절 동작은 로컬·106 임베드 양쪽 완료·검증됨(2026-07-20).** 이 항목은 "동작이 안 됨" TODO 가 아니라, 리뉴얼이 다른 앱과 **폰트 체계가 미세하게 다른** 점의 향후 통일 여지 기록.

**배경:** 리뉴얼(advisor-renual)은 폰트가 **px 하드코딩 위계 디자인**(제목 24 / 본문 13 등). 포털(host)은 폰트 조절 시 `<html>` 에 `--font-size-*` 토큰의 `-dynamic` 변형(예 `--font-size-body1-dynamic` 16→20)을 실시간 세팅한다(MCP 실측). 리뉴얼 px 는 이 토큰을 안 봐서 임베드에서 조절 안 됐던 것.

**적용된 최종 방식(현 상태 = 동작함):**
1. `src/view/advisor-renual/` px **345곳** → `font-size: var(--fs-N)` 치환.
2. `src/styles/global.scss`: `--fs-N: calc(Npx * var(--fs-scale))` + **`--fs-scale: calc(var(--font-size-body1-dynamic, 16px) / 16px)`**.
   - 임베드: 포털이 세팅하는 body1-dynamic/16 비율 = 배율 → **CSS 만으로 포털 글자크기에 자동 연동**(switchFont 미경유여도 동작).
   - 로컬: 포털 토큰 없어 fallback 1 → `useTheme.ts switchFont()` 가 `--fs-scale` setProperty(작게 0.75 / 일반 1 / 크게 1.25)로 반응.
3. 배율 0.75/1/1.25 = 포털 body1(12/16/20) 비율과 일치시켜 로컬·임베드 체감 통일.

**남은 통일 여지(선택):** 리뉴얼은 "px×배율(위계 유지)", 다른 앱은 "포털 절대 토큰(위계 없음/토큰 스케일)". 나란히 보면 미세하게 다를 수 있음.
- **완전 통일하려면** → 리뉴얼 px 를 포털 의미토큰(`--font-size-body/subtitle/heading/…-dynamic`)으로 **직접** 치환. ⚠️ 매핑이 수작업 판단 필요(px→의미역할) + **13px 는 포털 토큰 base 에 없어**(12/14 만 존재) 스케일 미스매치. 그래서 이번엔 "px 유지+배율 연동"으로 간 것.
- **또는** `--fs-scale` 기준 토큰만 바꿔 배율감 조정(예 subtitle4 기준 1.333). `global.scss --fs-scale` 정의 1곳만 손보면 리뉴얼 345곳 전체 전환됨(px 는 안 건드림 — 이게 변수로 모아둔 이유).

**되돌리기:** 위 1~3 되돌림. 치환 원복은 `var(--fs-N)` → `Npx` 역치환(sed).
