...
**필드 설명:**
- `type`: 장치 타입 ("system"|"mic")
- `aec_reset`: AEC(에코 제거) 리셋 여부 (macOS는 항상 false)
...
**참고:**
- 장치 제거 시에는 `MIC_DEVICE_LOST` 에러도 함께 전송됨
...
- 구현 정렬(C++/macOS): `sustained` 전송 후에도 무음 감지는 계속되어 유음 발생 시 `off` 전송. `off` 전송 직후 이번 세션의 무음 감지는 비활성화되어 추가 `off`가 반복되지 않습니다.
...
**장치 관련 오류:**
- `"MIC_DEVICE_LOST"`: 마이크 장치 제거/분리 감지 (macOS 전용)
  - 녹음 중일 때: 자동으로 녹음 중지
  - 대기/테스트 중일 때: 알림만 전송 (중지 없음)
  - 항상 `audio_device_change` 이벤트와 함께 전송됨
