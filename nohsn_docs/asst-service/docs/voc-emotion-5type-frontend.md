...
## 4. 프론트 체크리스트
- [ ] `emotion.type` 분기에 신규 5종(`angry/dissatisfied/normal/satisfied/thanks`) 아이콘·라벨 추가
- [ ] 레거시 3종(`negative/neutral/positive`) fallback 매핑 유지 (과거 콜이력 표시용)
- [ ] 미정의 값은 `normal`로 안전 처리
- [ ] `complaintRisk` / `churnRisk` 는 변경 없음 — 그대로 사용
